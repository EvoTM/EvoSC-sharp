    ...
    <template>
        ...
        <Window width="{{ w }}" height="{{ h }}" x="{{ -w/2 }}" y="{{ h/2 }}" title="Edit Message of the Day">
            ...
            <frame pos="0 {{ -h + buttonBarHeight*2 + 2 }}" size="{{ w }} {{ buttonBarHeight+2 }}" z-index="{{ zIndex + 1 }}">
                <Button text="Save" id="btnSave" action="MotdEditManialink/Save" x="{{ w/2-19 }}" width="18" />
    ...
    <script>
        ...
        ***
        if (Event.Control.ControlId == "button_save") {
            TriggerPageAction("MotdEditManialink/Save/"^(Page.GetFirstChild("textedit_text") as CMlTextEdit).Value);
